...
- FFmpeg 按固定时长分段录制，默认每段 15 分钟；归档时统一重新编码为 720p/30fps、视频 1.8 Mbps、音频 96 kbps。
- 临时录制使用 MPEG-TS 分段，断流或进程异常退出时仍可读取；最终归档仍合并为 MP4。
- 录制进程无新文件、异常退出或网络错误时自动重新获取直播地址并重拉流。
- 下播收尾有优雅停止和强制结束兜底，避免半写入 MP4 卡住后处理。
- 下播后先检查每个分段的字节数已稳定、可播放时长有效，再合并有效分段；合并 MP4 通过可播放性和时长校验后立即删除已参与合并的分段。
- 上传前再次检查合并录像的字节数已稳定，并核对合并时长与有效分段时长。
- 完整视频上传飞书后，通过分段下载请求核对云端与服务器的精确字节数；删除前再检查一次本地文件大小。任一校验失败都保留本场所有 MP4 以便重试。
- 有损坏分段，或有效录像时长明显短于直播时长时，标记为“部分录制”，并在失败原因中记录有效时长、缺失时长和损坏文件。
- 录制中的临时片段为带三位序号的 `.ts` 文件；最终合并录像为
  `直播视频-账号名称-直播时间_00.mp4`。TS 只用于抗中断和重连，原始分段不作为完成提醒链接。
- 下播后把妙记播放页写入“录制视频链接”；飞书只发送一条“直播录制完成提醒”，其中“录制视频”指向妙记播放页，“文字记录”和“智能纪要”分别指向飞书自动生成的两个 DOCX。
- 飞书云盘分片按接口要求串行上传，并保留断点续传检查点；飞书妙记单文件上限为 6 GB，超过时服务不会上传或重复提交，而是在直播记录中明确提示需拆分或压缩。
- 同一场次按“接收人 + 消息类型”持久化去重；发送中任务不能被其他工作进程重复领取，失败任务只重试失败的接收人。
- 同一场次的飞书妙记创建也使用持久化提交锁；正式任务和恢复任务并发时，只有一个进程可以调用创建接口，其余进程只能复用已保存的妙记链接。
...
| 最后开播时间 | 最近一场直播的开播时间 |
| 最后下播时间 | 最近一场直播的下播时间 |
| 监控场次 | 由系统自动累计；每创建一场新的直播记录时加一。计数以抖音号为主键保存在服务器状态库，服务重启或账号改名不会清零；首次同步时会保留表格中已有的较大值 |
...
`录制状态`、`转写状态`、`完成提醒状态`、`录制视频链接`、`智能纪要链接`、`文字记录链接`、`完成提醒时间`、
`失败原因`、`任务 ID`。
...
飞书云盘的单文件上传上限取决于企业套餐；飞书妙记 API 的单个音视频文件上限为 **6 GB**。
服务在飞书妙记上传前检查此限制：超过 6 GB 的录像保留在服务器，不会反复上传或创建妙记。
这类长直播需要先拆分为多个不超过 6 GB 的文件，或压缩/提取音频后再提交转写。
...
| `offline_confirmations` | 连续几次确认未开播后才结束场次 | `3` |
...
每场录像只有在飞书云端字节数与服务器合并文件完全一致后，才会自动删除该场的本地分段和合并文件。校验失败的录像会保留，长期运行仍应监控磁盘剩余空间。
