...
当前线上监控服务的生效配置仍来自服务器上的 `live_digest.json`；修改该文件后需要重启
`live-digest.service`。飞书多维表格用于维护账号资料、接收人和每场直播的结果记录，当前已支持
写入直播记录、录制状态、转写状态、推送状态、截图和逐字稿附件。

推荐的管理方式是：在“监控账号列表”维护账号和接收人，在“直播记录”查看每一场直播的处理结果。
当前版本尚未提供“修改表格后自动同步服务器配置”的程序，因此表格变更后仍需同步修改
`live_digest.json`；后续接入同步程序时应保持下面的字段和生效规则。
...
2. 将变更同步到服务器 `live_digest.json`：`rooms` 填写启用账号的直播间 URL，
   `feishu_recipients` 填写接收人及正确的 `id_type`。
3. 重启服务使配置生效：
...
当前版本不会因为编辑表格而自动发送消息；“直播记录”由服务或补录脚本写入，不应通过手工新增记录
来触发推送。后续实现表格同步时，建议使用抖音号作为唯一键，并按任务创建时间保存账号和接收人快照。
...
- 飞书多维表格变更目前不会自动改写服务器配置；账号或接收人变更后，仍需同步更新
  `live_digest.json` 并重启服务。
