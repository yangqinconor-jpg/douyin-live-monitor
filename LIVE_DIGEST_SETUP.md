...
`live_digest_service.py` 按配置监控一个或多个抖音直播间，默认每场直播按 900 秒分段录制。开播立即发送飞书文本通知；下播后合并为完整 MP4，归档到飞书云盘并交由飞书妙记转写，再发送一条包含录像、文字记录和智能纪要链接的完成提醒。
...
- **监控账号列表**：维护账号名称、抖音号（唯一标识）、直播间链接、监控开关和监控接收人。
- **直播记录**：每场直播一行，记录录制、转写、完成提醒状态，以及录制视频、智能纪要和文字记录链接。

服务会每 120 秒自动读取“监控账号列表”，同步启用账号、直播间链接和接收人。修改账号或接收人后
无需手工改 `rooms`，下一轮同步即可生效；正在执行的任务继续使用创建时的接收人快照。
只有修改 App Token、数据表 ID 等基础配置时才需要重启服务。手工补录直播记录不会触发新的推送。
账号名称可以修改，但抖音号不能修改。
